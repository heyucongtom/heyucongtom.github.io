...
Tensorflow is using a synchronization implementation which focuses more on stability and efficiency, and as a result becomes less controlable. This guide is intended as a in-depth guide on synchronous/asynchronous/custom synchronization for Tensorflow networks. The target audience is students and researchers who need to set up a fine-grained, controlled distributed Tensorflow environment. 
...
However, on networks with relatively simpler structure (which sometimes means there are less local minimums to trap the gradient descent, although not usually true), both asyn and async converges to the same performance.
...
This setup serves as an example on custom network training architecture using Tensorflow. There is still a lots of potential from customizing the training architecture, and associates/manages it via Kubernetes or similar cluster managers.
...
### Attachment:
0. Hybrid figure from “Kurth T, Zhang J, Satish N, Racah E, Mitliagkas I, Patwary MM, Malas T, Sundaram N, Bhimji W, Smorkalov M, Deslippe J. Deep learning at 15PF: supervised and semi-supervised classification for scientific data. InProceedings of the International Conference for High Performance Computing, Networking, Storage and Analysis 2017 Nov 12 (p. 7). ACM.”
